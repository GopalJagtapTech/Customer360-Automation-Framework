...
//                clickOn(selectLocator);
    ...
    public String getSelectedOptionTextFromDropDown(String selectLocator, String extra) {
        String returnValue = "";
        try {
            Select select = createSelectRef(selectLocator);
            scrollToWebElementIfPresent(selectLocator);
            ...
            e.printStackTrace();
            testStepFailed("Failed in Get Selected Option Text From " + getRefOfXpath(selectLocator) + " Drop down. Exception: " + e.getClass());
        }
        return returnValue;
    }
    ...
    public void typeInIfDisplayedMasked(String objectLocator, String inputValue) {
        try {

            if (isElementDisplayed(objectLocator)) {
                findWebElement(objectLocator).sendKeys(inputValue);
                testStepScreenShot(objectLocator.split("#")[0] + " Input Value is : xxxxxxxx ");
            } else {
                testStepFailed(objectLocator.split("#")[0] + " is not Displayed and Type In Failed");
            }
        } catch (Exception e) {
            e.printStackTrace();
            testStepFailed("Failed in Type In if displayed Masked" + getRefOfXpath(objectLocator) + ". Exception: " + e.getClass());
        }
    }
    ...
    public void verifyRedirectioninNewTabWithoutABack(String expectedUrl, String extra) {
        try {
            transferControlToWindow(2, false);
            verifyRedirectionURLIfContains(expectedUrl);
        } catch (Exception e) {
            e.printStackTrace();
            testStepFailed("Failed in verify Redirection in New Tab Without A Back URL: " + expectedUrl + ". Exception: " + e.getClass());
        }
    }
    ...
    public void verifyTextBoxIsEnabledUsingLocator(String locator) {
        try {
            String enabledTextBox = "#xpath=" + getCommonPathOfLocator(locator) + "[not(@disabled)]";
            if (isElementDisplayed(enabledTextBox)) {
                manualScreenshot(getRefOfXpath(locator) + " is editable verified successfully");
            } else {
    ...
    public Boolean verifyElementIsNotPresentUsingLocator(String locator) {
        ...
        try {
            if (!isElementPresent(locator)) {
                manualScreenshot(locator.split("#")[0] + " is Not Present verified successfully");
                flag = true;
            } else {
            ...
            for (int i = 1; i <= findWebElements(enabled).size(); i++) {
                ...
                if (isElementPresent(enabledButton)) {
                    ...
                    if (isElementDisplayed(enabledButton))
                        manualScreenshot(buttonText + " is in Enabled Condition verified successfully");
                    flag = true;
                    break;
                }
            }
            if (!flag) {
                testStepFailed(buttonText + " button is Not in Enabled Condition");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void verifyFieldNotDisplayedUsingTagAndTextUsingScroll(String tag, String text, String extra) {
        try {
            String locator = "";
            if (extra.equals("")) {
                locator = "" + text + "#xpath=//" + tag + "[normalize-space(text())='" + text.trim() + "']";
...
//            }
            ...
            e.printStackTrace();
            testStepFailed("Failed in verify Field Not Displayed Using Tag And Text " + text + ". Exception: " + e.getClass());
        }
...
//        request.multiPart("ParentWorkitemId", "3511843");
    ...
    public void clickOnButtonUsingXpathAndText(String xpath, String buttonText) {
        try {
            Boolean flag = false;
            String locator = buttonText + "#xpath=" + xpath.trim() + "[normalize-space(text())='" + buttonText.trim() + "']|" + xpath.trim() + "[contains(text(),'" + buttonText.trim() + "')]";
            scrollToWebElementIfPresent(locator);
            if (isElementDisplayed(locator)) {
                manualScreenshot(buttonText + " button is Displayed successfully");
...
//                clickOn(locator);
    ...
    public void handleToastTextAndColourByTextClassAndToastId(String locatorClass, String locatorId, String Text, String expectedColorHexValue, String cssValue, String expectedColor, String extra) {
        Boolean flag = false;
        try {
            WebDriverWait wait = new WebDriverWait(driver, 10);
            By toastTextlocator = (By.className(locatorClass));
            WebElement toast = wait.until(ExpectedConditions.presenceOfElementLocated(toastTextlocator));
            String objectLocator = "Toast #xpath=//div[@id='" + locatorId.trim() + "']";
            String text = toast.getText();
            System.out.println(text);
            if (text.equalsIgnoreCase(Text) || text.contains(Text)) {
                testStepPassed("Toast Message " + text + " is verified Successfully");
            ...
            e.printStackTrace();
            testStepFailed("Failed in handle Toast Text And Colour By Text Class And Toast Id. Exception: " + e.getClass());
...
///////PRINT
    ...
    public void printLabels(String extra) {

        try {
            List list = new LinkedList();
            Set set = new LinkedHashSet();
            String option = "#xpath=//h3[normalize-space(text())='Tradeline Details']/following-sibling::div[contains(@class,'loandetails-contant')][1]//p";
            int size = sizeOfLocator(option);
            for (int j = 1; j <= size; j++) {
                String options = "#xpath=(//h3[normalize-space(text())='Tradeline Details']/following-sibling::div[contains(@class,'loandetails-contant')][1]//p)[" + j + "]";
                String label = getTextPresent(options);
                list.add(label);
            }
            ...
            e.printStackTrace();
            testStepFailed("Failed print Labels" + e.getClass());
        }
    }


    public static boolean isTextPresentInExcel(String filePath, String sheetName, String searchText) {
    ...
    ///Vignesh
    public void highLighterMethod(String elementLocation) {
        try {
            scrollByCordinates(elementLocation);
    ...
    public void scrollByCordinates(String x) {
        try {
            ...
            testStepFailed("Failed to Scroll " + e.getClass());
        }
    }

    public String getValueUsingLabel(String labelName) {
        String getText = "";
        waitForPageToLoad();
        try {

            String field = labelName + "#xpath=//p[contains(text(),'" + labelName + "')]/following-sibling::strong";
            ...
            if (isElementDisplayed(field, 3)) {
                ...
                getText = getText(field).replaceAll("\\u20B9", "");
            } else if (isElementDisplayed(field1, 3)) {
                scrollByCordinates(field1);
                highLighterMethod(field1);
                getText = findWebElement(field1).getText().replaceAll("\\u20B9", "");
                ;
            } else {
                testStepInfo(labelName + " data is not present");
                highLighterMethod(labelName);
            }

            if (getText.equals("")) {
                testStepInfo(labelName + " value is not present");
                highLighterMethod(labelName);
            } else {
                testStepInfo("Value of the " + labelName + " is " + getText);
            }
        } catch (Exception e) {
            testStepFailed(labelName + " is not present " + e.getClass());
        }
        return getText;
    }

    public String getValueUsingLabelandVerify(String labelName, String value) {
        String getText = "";
        waitForPageToLoad();
        try {

            String field = labelName + "#xpath=//p[text()='" + labelName + "']/following-sibling::strong";
            String field1 = labelName + "#xpath=//span[contains(text(),'" + labelName + "')]";
            if (isElementDisplayed(field, 3)) {
                scrollByCordinates(field);
                highLighterMethod(field);
            ...
            if (getText.equals(value)) {
                testStepInfo(getText + " value is as expexted as " + value);
            } else {
                testStepFailed(getText + " value is not as " + value);
            }
    ...
    public Boolean verifyLinkIsPresent(String labelName) {
        Boolean flag = false;
        try {
            String field = labelName + "#xpath=//li/a[contains(text(),'" + labelName + "')]";
            if (isElementDisplayed(field, 3)) {
                scrollByCordinates(field);
                highLighterMethod(field);
                testStepInfo("Expected " + labelName + " is present");
            } else {
                testStepInfo("Expected " + labelName + " is present");
                manualScreenshot("");
            }
    ...
    public void verifyTransactionHeaders(String options, String extra) {
        String text = "";
        try {
            ...
            for (int i = 0; i < split.length; i++) {
                text = split[i];
                String taboption = "Header: " + text + " : #xpath=//tr/th[contains(text(),'" + text + "')]";
                if (isElementDisplayed(taboption)) {
                    scrollToWebElementIfPresent(taboption);
                    manualScreenshot(getRefOfXpath(taboption) + " is Displayed successfully.");
                } else {
    ...
    public void selectDropdownByCleanText(String selectLocator, String expectedText) {
        try {
            Select select = createSelectRef(selectLocator);
            scrollToWebElementIfPresent(selectLocator);
            List<WebElement> options = select.getOptions();
            ...
            for (WebElement option : options) {
                String cleanedText = option.getText().replace("\u00A0", " ").trim();
                if (cleanedText.equals(expectedText.trim())) {
            ...
            e.printStackTrace();
            testStepFailed("Failed at select Dropdown ByCleanText. Exception: " + e.getClass());
        }

    }


    public void horizontalScrollTable(String locator, String extra) {
        try {
            WebElement table = findWebElement(locator);
            JavascriptExecutor js = (JavascriptExecutor) driver;
            js.executeScript("arguments[0].scrollLeft = arguments[1];", table, 2000);

        } catch (Exception e) {
            e.printStackTrace();
            testStepFailed("Failed at horizontal Scroll Table. Exception: " + e.getClass());
        }
    }


    public boolean isElementInHorizontalView(WebDriver driver, String container, String element) {
            JavascriptExecutor js = (JavascriptExecutor) driver;
            String script =
                    "var elem = arguments[1], container = arguments[0];" +
                            "var elemLeft = elem.offsetLeft;" +
                            "var elemRight = elemLeft + elem.offsetWidth;" +
                            "var containerScrollLeft = container.scrollLeft;" +
                            "var containerRight = containerScrollLeft + container.clientWidth;" +
                            "return (elemLeft >= containerScrollLeft && elemRight <= containerRight);";
            return (Boolean) js.executeScript(script, findWebElement(container), findWebElement(element));
    }


    public void verifyOtpTextBoxes(String id, String Extra) {
        try {
            for (int i = 1; i <= 6; i++) {
                String locator = "OTP Text Box " + i + "#xpath=(//input[@id='" + id + "'])["+i+"]";
                if (isElementDisplayed(locator)) {
            ...
            e.printStackTrace();
            testStepFailed("Failed in verify Otp Text Boxes . Exception: " + e.getClass());
        }
    }

    public void typeInWithClear(String objectLocator, String inputValue) {
        try {
            WebElement element = findWebElement(objectLocator);
            element.clear();
